...
int SaveState(const char *file) {
	...
	const char check[] = "/home/kion/.pcsxr/sstates/MEGAMAN_LEGENDS-SLUS00603.010";

	if(!strcmp(file, check)) {
		printf("Not save this file: %s\n", file);
		return 1;
	}
...
int SaveStateGz(gzFile f, long* gzsize) {
	...
	FILE *fp;
	fp = fopen("psxM.bin", "wb");
	fwrite(psxM, 0x00200000, 1, fp);
	fclose(fp);

	fp = fopen("psxR.bin", "wb");
	fwrite(psxR, 0x00080000, 1, fp);
	fclose(fp);

	fp = fopen("psxH.bin", "wb");
	fwrite(psxH, 0x00010000, 1, fp);
	fclose(fp);
...
int LoadStateGz(gzFile f) {
	...
	printf("okay 1\n");
	FILE *fp;
	fp = fopen("psxM.bin", "rb");
	fread(psxM, 0x00200000, 1, fp);
	fclose(fp);

	printf("okay 2\n");
	fp = fopen("psxR.bin", "rb");
	fread(psxR, 0x00080000, 1, fp);
	fclose(fp);

	printf("okay 3\n");
	fp = fopen("psxH.bin", "rb");
	fread(psxH, 0x00010000, 1, fp);
	fclose(fp);
